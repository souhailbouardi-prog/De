    ...
    def __init__(
        ...
        if not skip_memory:
            try:
                ...
                # Check if a plugin already registered a memory provider via PluginContext
                try:
                    from hermes_cli.plugins import get_plugin_memory_provider
                    _plugin_mp = get_plugin_memory_provider()
                    if _plugin_mp is not None:
                        from agent.memory_manager import MemoryManager as _MemoryManager
                        self._memory_manager = _MemoryManager()
                        if _plugin_mp.is_available():
                            self._memory_manager.add_provider(_plugin_mp)
                        # Skip the built-in plugins/memory/ lookup
                        _mem_provider_name = ""
                except Exception:
                    pass
