...
class PluginContext:
    ...
    def register_memory_provider(self, provider) -> None:
        """Register a memory provider from a plugin.

        The provider must be an instance of ``agent.memory_provider.MemoryProvider``.
        Only one memory provider plugin is allowed. If a second plugin tries to
        register one, it is rejected with a warning.
        """
        if self._manager._memory_provider is not None:
            logger.warning(
                "Plugin '%s' tried to register a memory provider, but one is "
                "already registered by plugin '%s'. Only one memory provider "
                "plugin is allowed.",
                self.manifest.name,
                self._manager._memory_provider_name,
            )
            return
        ...
        self._manager._memory_provider_name = self.manifest.name
        logger.info(
...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        self._memory_provider = None  # Set by a plugin via register_memory_provider()
        self._memory_provider_name: Optional[str] = None
...
def get_plugin_memory_provider():
    ...
    return get_plugin_manager()._memory_provider
