...
def test_search_fiscal_sources_non_article_query_keeps_fallback_path(monkeypatch):
    ...
    assert payload.get("tool_name") != "get_code_article_version"
    assert "code_article_version_not_verified" not in serialized
    assert (
        "fallback search result" in serialized
        or "mcp_openlegi_rechercher_code" in serialized
