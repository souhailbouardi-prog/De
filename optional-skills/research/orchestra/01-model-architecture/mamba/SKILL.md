...
description: "State-space model with O(n) complexity vs Transformers' O(n\xB2). 5\xD7\
  \ faster inference, million-token sequences, no KV cache. Selective SSM with hardware-aware\
...
metadata:
  hermes:
    category: research/orchestra/01-model-architecture
    tags:
    - Model Architecture
    - Mamba
    - State Space Models
