...
sidebar_label: "上下文引用"
title: "上下文引用"
description: "使用内联 @ 语法直接在消息中附加文件、文件夹、git diff 和 URL"
---

# 上下文引用

输入 `@` 后跟引用，将内容直接注入到您的消息中。Hermes 会内联展开引用，并在 `--- 附加上下文 ---` 部分下追加内容。
...
`@file:` 引用支持行范围，用于精确内容注入：

```text
@file:src/main.py:42        # 单行 42
@file:src/main.py:10-25     # 行 10 到 25（包含两端）
```
...
| Git 提交 | 最多 10 个 | `@git:N` 限制在范围 [1, 10] 内 |

## 安全性

### 敏感路径阻止

以下路径始终被 `@file:` 引用阻止，以防止凭证泄露：
...
上下文引用主要是 **CLI 功能**。它们在交互式 CLI 中工作，其中 `@` 触发标签补全，引用在消息发送到代理之前展开。

在 **消息平台**（Telegram、Discord 等）中，`@` 语法不会被网关展开 — 消息会原样传递。代理本身仍然可以通过 `read_file`、`search_files` 和 `web_extract` 工具引用文件。
